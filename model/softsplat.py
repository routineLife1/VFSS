...
def softsplat(tenIn:torch.Tensor, tenFlow:torch.Tensor, tenMetric:torch.Tensor, strMode:str):
    ...
    if strMode == 'avg':
        ...
        # tenMetricMax = softsplat_zmax_func.apply(tenMetric.detach(), tenFlow.detach())
            ...
            cuda_launch(cuda_kernel('softsplat_out', '''
                extern "C" __global__ void __launch_bounds__(512) softsplat_out(
                    ...
                    for (int i = intNorthwestX - 1; i < intNorthwestX + 3; i++)
                    {
                        ...
                        {
                            {{type}} dis2 = (i - fltX) * (i - fltX) + (j - fltY) * (j - fltY);
                            {{type}} weight = exp(-dis2) / 0.72;  // exp(-q^2/2sigma^2), sigma = 0.6
                            if ((i >= 0) && (i < SIZE_3(tenOut)) && (j >= 0) && (j < SIZE_2(tenOut))) {
                                atomicAdd(&tenOut[OFFSET_4(tenOut, intN, intC, j, i)], fltIn * weight);
                            }
                        }
                    } 
                    */
                ...
                extern "C" __global__ void __launch_bounds__(512) softsplat_ingrad(
                    ...
                    for (int i = intNorthwestX - 1; i < intNorthwestX + 3; i++)
                    {
                        for (int j = intNorthwestY - 1; j < intNorthwestY + 3; j++)
                        {
                            {{type}} dis2 = (i - fltX) * (i - fltX) + (j - fltY) * (j - fltY);
                            {{type}} weight = exp(-dis2) / 0.72;  // exp(-q^2/2sigma^2), sigma = 0.6
                            if ((i >= 0) && (i < SIZE_3(tenOutgrad)) && (j >= 0) && (j < SIZE_2(tenOutgrad))) {
                                fltIngrad += VALUE_4(tenOutgrad, intN, intC, j, i) * weight;
                            }
                        }
                    } 
                    */
                    ...
                    for (int intChannel = 0; intChannel < SIZE_1(tenOutgrad); intChannel += 1) {
                        ...
                        for (int i = intNorthwestX - 1; i < intNorthwestX + 3; i++)
                        {
                            for (int j = intNorthwestY - 1; j < intNorthwestY + 3; j++)
                            {
                                {{type}} dis2 = (i - fltX) * (i - fltX) + (j - fltY) * (j - fltY);
                                {{type}} weight = exp(-dis2) / 0.72;  // exp(-q^2/2sigma^2), sigma = 0.6
                                {{type}} weight_grad = 0.0f;
                                if (intC == 0) { // x
                                    weight_grad = weight * (-2.0f) * (i - fltX); // dweight/dx
                                } else { // y
                                    weight_grad = weight * (-2.0f) * (j - fltY); // dweight/dy
                                }
                                if ((i >= 0) && (i < SIZE_3(tenOut)) && (j >= 0) && (j < SIZE_2(tenOut))) {
                                    fltFlowgrad += VALUE_4(tenOutgrad, intN, intChannel, j, i) * fltIn * weight_grad;
                                }
                            }
                        } 
                        */
