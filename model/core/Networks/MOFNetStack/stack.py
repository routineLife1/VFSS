...
zeros_cache = {}
def get_zeros_cache(size, i, device, dtype):
    k = (str((device, dtype)), str(size), str(i))
    if k in zeros_cache:
        return zeros_cache[k]
    t = torch.zeros(*size).to(device=device, dtype=dtype)
    zeros_cache[k] = t
    return t
...
class SKMotionEncoder6_Deep_nopool_res(nn.Module):
    ...
    def forward(self, motion_hidden_state, forward_flow, backward_flow, coords0, forward_corr, backward_corr, bs):
        ...
        forward_motion_hidden_state = torch.cat([motion_hidden_state[:, 1:, ...], get_zeros_cache((bs, 1, 48, H, W), 'fwd', device=motion_hidden_state.device, dtype=motion_hidden_state.dtype)], dim=1).reshape(BN, -1, H, W)
        forward_motion_hidden_state = self.sample_flo_feat(forward_loc, forward_motion_hidden_state)
        backward_motion_hidden_state = torch.cat([get_zeros_cache((bs, 1, 48, H, W), 'bwd', device=motion_hidden_state.device, dtype=motion_hidden_state.dtype), motion_hidden_state[:, :N-1, ...]], dim=1).reshape(BN, -1, H, W)
