...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            k = (str((coords.device, coords.dtype)), str((r)))
            if k in delta_lvl_cache:
                delta_lvl = delta_lvl_cache[k]
            else:
                dx = torch.linspace(-r, r, 2 * r + 1)
                dy = torch.linspace(-r, r, 2 * r + 1)
                delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(device=coords.device, dtype=coords.dtype, non_blocking=True)
                delta_lvl = delta.view(1, 2 * r + 1, 2 * r + 1, 2)
                delta_lvl_cache[k] = delta_lvl
    ...
    def corr(fmap1, fmap2):
        batch, dim, ht, wd = fmap1.shape
        k = (str((fmap1.device, fmap1.dtype)), str(dim))
        if k in dim_cache:
            dim_t = dim_cache[k]
        else:
            dim_t = torch.sqrt(torch.tensor(dim).to(fmap1.device, fmap1.dtype))
