...
class MOFNet(nn.Module):
    ...
    def initialize_flow(self, img, bs, down_ratio):
        ...
        N, C, H, W = img.shape
        coords0 = coords_grid(bs, H // down_ratio, W // down_ratio, device=img.device, dtype=img.dtype)
        coords1 = coords_grid(bs, H // down_ratio, W // down_ratio, device=img.device, dtype=img.dtype)
    ...
    def forward(self, images, data={}, flow_init=None):
        ...
        forward_corr_fn = CorrBlock(fmaps[:, 1:N - 1, ...].reshape(B * (N - 2), -1, H // down_ratio, W // down_ratio),
                                    fmaps[:, 2:N, ...].reshape(B * (N - 2), -1, H // down_ratio, W // down_ratio),
                                    num_levels=self.cfg.corr_levels, radius=self.cfg.corr_radius)
        backward_corr_fn = CorrBlock(fmaps[:, 1:N - 1, ...].reshape(B * (N - 2), -1, H // down_ratio, W // down_ratio),
        ...
        for itr in range(self.cfg.decoder_depth):
            ...
            flow_predictions.append(
                torch.cat([forward_flow_up.reshape(B, N - 2, 2, H, W), backward_flow_up.reshape(B, N - 2, 2, H, W)],
                          dim=1))
